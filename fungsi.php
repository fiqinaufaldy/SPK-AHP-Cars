...
function getKriteriaNama($no_urut)
{
	...
	$nama = array(); // Initialize the $nama variable as an empty array

	while ($row = mysqli_fetch_array($result)) {
		$nama[] = $row['nama'];
	}

	if (!empty($nama) && array_key_exists($no_urut, $nama)) {
		return $nama[$no_urut];
	}

	return null; // Return null or handle the case when the index is out of bounds
}
...
function getAlternatifPV($id_alternatif, $id_kriteria)
{
	...
	// Initialize $pv with a default value ( Biar tidak terjadi error undefined variable php )
	// kasih value null biar nilai jadi default.
	$pv = null;
...
function getNilaiIR($jmlKriteria)
{
	...
	$result = mysqli_query($koneksi, $query);
	$nilaiIR = null; // Initialize with a default value
	while ($row = mysqli_fetch_array($result)) {
		$nilaiIR = $row['nilai'];
	}
...
{
	$eigenvektor = getEigenVector($matrik_a, $matrik_b, intval($n));
	$denominator = intval($n) - 1;

	if ($denominator != 0) {
		$consindex = ($eigenvektor - intval($n)) / $denominator;
		return $consindex;
	} else {
		/**
		 * Return an appropriate value or error message
		 * For example, can return null or throw an exception
		 */
		return null;
	}
}
...
function getConsRatio($matrik_a, $matrik_b, $n)
{
	...
	// Biar tidak ada error " Uncaught DivisionByZeroError "
	if ($nilaiIR != 0) {
								...
								<tr>
									...
									<td>
										<div class="field">
											...
											?>
											<input type="text" name="bobot<?php echo $urut ?>" class="form-control" type="text" value="<?php echo $nilai ?>" required id="html5-text-input" />
										</div>
									</td>
								</tr>
						<?php
